...
class DefaultContextManager(
    ...
    override suspend fun enhanceContext(context: String): String {
        ...
        recordContextOperation("enhance", mapOf("original_length" to context.length, "enhanced_length" to enhanced.length))
    ...
    override suspend fun enableCreativeMode() {
        ...
        recordContextOperation("enable_creative_mode", mapOf("timestamp" to System.currentTimeMillis()))
    }
    
    override suspend fun enableUnifiedMode() {
        isUnifiedModeEnabled = true
        updateContext("unified_mode", true)
        recordContextOperation("enable_unified_mode", mapOf("timestamp" to System.currentTimeMillis()))
    }
    ...
    override suspend fun recordInsight(request: String, response: String, complexity: String) {
        ...
        val insight = "Request complexity: $complexity. Key patterns: ${extractKeywords(request + " " + response).take(3)}"
        val timestamp = System.currentTimeMillis()
        
        memoryManager.storeMemory("insight_$timestamp", insight)
        
        recordContextOperation("record_insight", mapOf(
    ...
    override fun updateContext(key: String, value: Any) {
        ...
        recordContextOperation("update", mapOf("key" to key, "value_type" to value.javaClass.simpleName))
    }
